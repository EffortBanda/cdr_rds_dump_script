...
#
#
if [ ! -f /var/www/BHT-EMR-API/config/database.yml ]; then
...
#
#
parse_yaml /var/www/BHT-EMR-API/config/database.yml > database_yaml_values.txt
...
if [[ "$db" == "" ]]; then
   ...
   echo "ERROR : No database configured for development in \"/var/www/BHT-EMR-API/config/database.yml\".. Process exiting.."
   exit 1
else
   echo "SUCCESS : $db configured in \"/var/www/BHT-EMR-API/config/database.yml\"... Checking if MySQL service is running "
fi
